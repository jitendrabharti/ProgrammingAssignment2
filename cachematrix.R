## This program computes the inverse of a matrix by using caching rather than
## computing it repeatedly. If the contents of a matrix have not changed, it
## caches the inverse of the matrix so that it can be looked up in the cache 
## rather than recomputing it. The Program takes advantage of the scoping rules of
## R language & how they can be manipulated to preserve state inside of an R object.
...
makeCacheMatrix <- function(x = matrix()) {
     ...
     ## Create a List with all the functions and name them accordingly so that it is convenient to call    
